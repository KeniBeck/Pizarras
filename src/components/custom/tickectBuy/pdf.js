...
const generatePDF = async (tickets, fecha) => {
  ...
  tickets.forEach((data, index) => {
    ...
    doc.text(`N${data[0].Idsorteo}`, 10, yPosition);
    doc.setTextColor(0, 0, 0);
    doc.setFont("helvetica", "normal");
    doc.text(`Costo $ ${data[0].Costo}`, 10, yPosition + 10);
    doc.text(`Número de boleto: ${data[0].Boleto}`, 10, yPosition + 20);
  ...
  doc.setFont("helvetica", "bold");
  let leyenda = tickets[0].leyenda.leyenda1;
