...
const LoginWinner = () => {
    const { loginWinner, getUserData } = useSession();
    ...
    const enviarDatos = async (dataUser) => {
        ...
        await fetch("/api/login/winner", options)
            .then(res => res.json())
    ...
    return (
        ...
        <form onSubmit={handleSubmit(enviarDatos)} className="max-w-sm mx-auto w-full ">
            ...
            <div className="mb-6 text-white flex justify-center items-center text-xl">Bienvenido Administrador</div>
